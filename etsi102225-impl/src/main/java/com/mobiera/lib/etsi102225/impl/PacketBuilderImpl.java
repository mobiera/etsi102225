...
import com.mobiera.lib.etsi102225.api.model.CommandSPI;
...
import com.mobiera.lib.etsi102225.api.model.ResponsePacketHeader;
...
import com.mobiera.lib.etsi102225.impl.coders.CommandSPICoder;
import com.mobiera.lib.etsi102225.impl.coders.KICCoder;
import com.mobiera.lib.etsi102225.impl.coders.KIDCoder;
import com.mobiera.lib.etsi102225.impl.coders.ResponsePacketStatusCoder;
import com.mobiera.lib.etsi102225.impl.coders.ResponseSPICoder;
import com.mobiera.lib.etsi102225.impl.crypto.CipheringManager;
import com.mobiera.lib.etsi102225.impl.crypto.SignatureManager;
...
public class PacketBuilderImpl implements PacketBuilder
{
	...
	private CardProfile copyProfile(CardProfile profile) throws Etsi102225Exception
	{
		try
		{
		...
		{
			throw new Etsi102225Exception(ex);
		}
	}
...
//FIXME			throw new PacketBuilderConfigurationException("KID CertificationAlgorithmMode cannot be null");
		...
		switch (kid.getAlgorithmImplementation())
		{
			...
			case CRC:
				switch (kid.getCertificationAlgorithmMode())
				{
					case CRC16:
					case CRC32:
						signatureAlgorithmName = SignatureManager.CRC32;
						break;
			...
			case AES:
				...
				{
					case AES_CMAC4:
						signatureAlgorithmName = SignatureManager.AES_CMAC4;
						break;
						
					case AES_CMAC8:
						signatureAlgorithmName = SignatureManager.AES_CMAC8;
						break;
	...
	@Override
	public byte[] buildCommandPacket(byte[] data, long counters, byte[] cipheringKey, byte[] signatureKey)
			throws PacketBuilderConfigurationException, Etsi102225Exception
	{
		...
		// Convert 5 least significant bytes to a byte array
		byte [] cntrBytes = longToBytes(counters);
		cntrBytes = Arrays.copyOfRange(cntrBytes, cntrBytes.length - COUNTERS_SIZE, cntrBytes.length);
		
		if (LOGGER.isDebugEnabled())
			LOGGER.debug("Creating command packet.\n\tData:" + Util.toHexArray(data) + "\n\tCounters:"
					+ Util.toHexArray(cntrBytes) + "\n\tCipheringKey:" + Util.toHexArray(cipheringKey) + "\n\tSigningKey:"
		...
		try
		{
			...
			//byte[] countersBytes = usingCounters ? cntrBytes.clone() : new byte[COUNTERS_SIZE];
			byte[] countersBytes = cntrBytes != null ? cntrBytes.clone() : new byte[COUNTERS_SIZE];
			...
			if (commandPacketCiphering)
			{
				...
				// BUGFIX: iv shall be always 00..00 (according to 03.48 and then clarified in 23.048)
				
				byte [] iv = new byte[8];
				byte [] cipheredData = CipheringManager.encipher(cipheringAlgorithmName, cipheringKey, cipherData, iv);
		...
		{
			throw new Etsi102225Exception(e);
		}
	}
			...
			throws PacketBuilderConfigurationException, Etsi102225Exception
	{
		...
		if (data.length - PACKET_LENGHT_SIZE != packetLength)
			throw new Etsi102225Exception("Length of raw data doesnt match packet length. Expected " + packetLength + " but found "
		...
		if (data.length < MINIMUM_RESPONSE_PACKET_SIZE + signatureLength)
		{
			...
			throw new Etsi102225Exception(message);
		}
...
//				End of modification by Tomas Andersen / Morecom AS 2014.04.08 - TEST CASE: Tomas Andersen Bug #1->
			...
			if (responsePacketSigning)
			{
				...
				if (!valid)
					throw new Etsi102225Exception("Signatures doesnt match");
			}
	...
	{
		throw new Etsi102225Exception("Not implemented yet");
	}
